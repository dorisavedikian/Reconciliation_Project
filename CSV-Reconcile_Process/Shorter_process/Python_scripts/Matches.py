...
df1[['NALT_URI_Best_Match', 'randomint']] = df1['Nalt_URI'].str.split('_', expand=True)         
df2[['NALT_URI_Best_Match', 'randomint']] = df2['Nalt_URI'].str.split('_', expand=True)         
df_1 = df1.drop(['Nalt_URI','randomint'], axis=1) 
df_2 = df2.drop(['Nalt_URI','randomint'], axis=1)

df3 = df_2.loc[df_2['Score'] == 100]
df4 = df_2.loc[df_2['Score'] < 100]

df4 = df4.round({'Score': 0})
df5 = df4.sort_values(by=['Score'], ascending = False)
...
df5.to_excel(writer, sheet_name = 'Unmatched & < 100%', index=False) 
writer.close()
